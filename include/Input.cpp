...
void Input::makeConversions(bool enforce) {
    ...
    if ( iter->second.empty() ) {
        restartTime_ = -1; // s - Razer e Bruno - para retirar o timeout
    ...
    // Razer e Bruno
    // -pbs
    //iter = params_.find("-pbs");
    //pbs_ = iter->second.empty() ? false : (iter->second != "0");

    // Razer e Bruno
    // -pbs <modo>: 0 - cnf/mhf, 1 - pbs direto do cnf, 2 - pbs compactado
     iter = params_.find("-pbs");
	 pbs_ = 0;
     if (iter->second.empty() ) {
		 pbs_ = 0;
		 ...
		 Assert<Usage>(dummy = IsInteger(iter->second),
							  "Expected Int value for -pbs");
		 pbs_ = convert<long>(iter->second);
		 Assert<Usage>(pbs_ >= 0, "Expected +Int value for -pbs");
     }


     // Razer
    // -iscas
    iter = params_.find("-iscas");
    iscas_ = iter->second.empty() ? false : (iter->second != "0");

     // Razer
    // -mhf
    iter = params_.find("-mhf");
    mhf_ = iter->second.empty() ? false : (iter->second != "0");

    // Razer
     // -pure
     iter = params_.find("-pure");
     pure_ = iter->second.empty() ? false : (iter->second != "0");

     // Razer
      // -prefs
     iter = params_.find("-prefs");
	 prefs_ = 0;
     if (iter->second.empty() ) {
		 prefs_ = 0;
     }
		 ...
		 Assert<Usage>(dummy = IsInteger(iter->second),
							  "Expected Int value for -prefs");
		 prefs_ = convert<long>(iter->second);
		 Assert<Usage>(prefs_ >= 0, "Expected +Int value for -prefs");

		 options_ = options_ + " -pref-vars=prefs.txt ";
     }
