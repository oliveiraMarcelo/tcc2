...
struct CNFEncoding {
	...
	std::string style = convert<std::string>(input_.Encoding());
        ...
        /* Razer e Bruno*/
        if ( input_.PBS() )
        	args += (" -B " + convert<std::string>(input_.PBS()) + " " );  /* to generate PBS */
        	//args += (" -B 1");  /* to generate PBS */

        /* Razer */
        if ( input_.MHF() )
        	args += (" -h 1");  /* to generate MHF */

         /* Razer */
        if ( input_.ISCAS() )
        	args += (" -n 1");  /* to generate ISCAS */

        /* Razer */
        if ( input_.PURE() )
        	args += (" -u 1");  /* to generate MHF */

        /* Razer */
        if ( input_.PREFS() )
        	args += (" -e " + convert<std::string>(input_.PREFS()) + " " );  /* to generate PREFS */
